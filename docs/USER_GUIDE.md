...
### プレイヤーへのアタック（基本進行）とブロック選択
1. アタックしたいフロントラインのキャラを右クリック（またはカードメニュー）し、**「⚔️ アタック宣言（攻撃）」** を選択します（ターゲットラインが伸びます）。
2. 相手プレイヤーの盤面エリアまたはライフをクリックします。
3. アタッカーが自動的にレストになり、相手に **【ブロック選択】バー** が表示されます：
   - **ブロックする場合**: 防御側のフロントラインにいるアクティブ状態のキャラをクリックします。ブロッカーがレストになり、**バトル解決**へ移行します。
   - **通す場合**: **`[🛡️ 通す (ノーブロック)]`** をクリックします。
4. ノーブロック時、**【ライフダメージ】バー** が表示されます：
   - `[⚡ ライフトップをチェック]`: ライフの一番上のカードで自動トリガーチェック（公開モーダルが表示）。
   - `[📋 ライフ一覧から選択]`: ライフ選択モーダルを開き、任意のライフを指定してチェック。
   - 【インパクト】や【ダメージ②】等による追加ダメージは、サイドゾーンのライフ操作やライフ選択モーダルからいつでも自由に手動実行できます。

### キャラへのアタック（【狙い撃ち】/ バトル解決）
1. アタック宣言後、相手プレイヤーではなく**相手フロントラインのキャラを直接クリック**すると、【狙い撃ち】による直接バトルが発生します（テキスト記載・効果付与問わずいつでも直接攻撃可能）。
2. **公式ルール (Ver 1.1) に準拠したBP比較判定**が行われます：
   - **アタッカーBP $\ge$ 防御キャラBP**: アタッカー勝利。防御キャラのみが自動的に場外へ送られます（アタッカーは場に残ります。BP同値でもアタック側勝利）。
   - **アタッカーBP $<$ 防御キャラBP**: 防御キャラ勝利（ブロック成功）。両者とも場外には送られず、無傷でフィールドに残ります。
3. バトルログに詳細な勝敗とBP結果が出力されます。
